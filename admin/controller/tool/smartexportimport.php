...
class ControllerToolSmartExportImport extends Controller { 
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && ($this->validate())) {
			if ((isset( $this->request->files['upload'] )) && (is_uploaded_file($this->request->files['upload']['tmp_name']))) {
				...
				if ($this->model_tool_smartexportimport->upload($file)===TRUE) {
					...
					$this->response->redirect($this->url->link('tool/smartexportimport', 'token=' . $this->session->data['token'], 'SSL'));
				}
				else {
		...
		$data['action'] = $this->url->link('tool/smartexportimport', 'token=' . $this->session->data['token'], 'SSL');
		$data['export'] = $this->url->link('tool/smartexportimport/download', 'token=' . $this->session->data['token'], 'SSL');
		...
		$this->response->setOutput($this->load->view('tool/smartexportimport.tpl', $data));
	}
