...
class ControllerSaleOrder extends Controller {
		...
		if ($order_info) {
			...
			$data['shipping_address'] = str_replace(array("\r\n", "\r", "\n"), '<br />', preg_replace(array("/\s\s+/", "/\r\r+/", "/\n\n+/"), '<br />', trim(str_replace($find, $replace, $format))));
				...
				foreach ($data['shipping_custom_fields'] as $custom_field) {
					$data['shipping_address'] .= '(' . $custom_field['name'] . ':' . $custom_field['value'] . ')';
				}
	...
	public function invoice() {
		...
		$data['text_contacter'] = $this->language->get('text_contacter');
		$data['text_telephone'] = $this->language->get('text_telephone');
		$data['text_address'] = $this->language->get('text_address_1');
		$data['text_company_name'] = $this->language->get('text_company_name');
		$data['column_order_total'] = $this->language->get('column_order_total');
		$data['column_order_total_1'] = $this->language->get('column_order_total_1');
		$data['text_invoice_comment'] = $this->language->get('text_invoice_comment');

		$data['column_index'] = $this->language->get('column_index');
		$data['column_product'] = $this->language->get('column_product');
		$data['column_model'] = $this->language->get('column_model');
		$data['column_quantity'] = $this->language->get('column_quantity');
		$data['column_price'] = $this->language->get('column_price');
		...
		foreach ($orders as $order_id) {
			...
			if ($order_info) {
				...
				if ($order_info['shipping_address_format']) {
					...
					$format = '{firstname}' . " " . '{country}' . " " . '{zone}' . " " . '{city}' . " " . '{address_1}' . '{postcode}';
				}
				...
				$data['orders'][] = array(
					...
					'order_total_1'               => Num2cny::ParseNumber($order_info['total']),
					'shipping_address'    => $shipping_address,
	...
	public function shipping() {
		...
		$data['text_shipping_address'] = $this->language->get('text_shipping_address');
		$data['text_shipping_comment'] = $this->language->get('text_shipping_comment');
		$data['text_contacter'] = $this->language->get('text_contacter');
		$data['text_print'] = $this->language->get('text_print');
		...
		$data['column_order_total'] = $this->language->get('column_order_total');
		$data['column_shipping_fee'] = $this->language->get('column_shipping_fee');
		$data['column_price'] = $this->language->get('column_price');
		$data['column_total'] = $this->language->get('column_total');
