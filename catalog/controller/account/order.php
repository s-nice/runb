...
class ControllerAccountOrder extends Controller {
	...
	public function index() {
		...
		if(isset($this->request->get['type']) && $this->request->get['type'] == 'shipped'){
			$order_total = $this->model_account_order->getTotalOrdersOfStatusShipped();
			$results = $this->model_account_order->getOrdersOfStatusShipped(($page - 1) * 10, 10);
		}else if(isset($this->request->get['type']) && $this->request->get['type'] == 'unshipped'){
			$order_total = $this->model_account_order->getTotalOrdersOfStatusUnshipped();
			$results = $this->model_account_order->getOrdersOfStatusUnshipped(($page - 1) * 10, 10);
		}else if(isset($this->request->get['type']) && $this->request->get['type'] == 'unpaid'){
			$order_total = $this->model_account_order->getTotalOrdersOfStatus0();
			$results = $this->model_account_order->getOrdersOfStatus0(($page - 1) * 10, 10);
		}else{
			$order_total = $this->model_account_order->getTotalOrdersWithStatus0();
			$results = $this->model_account_order->getOrdersWithStatus0(($page - 1) * 10, 10);
		}
		...
		foreach ($results as $result) {
			...
			$data['orders'][] = array(
				...
				'status'     => trim($result['status'])!='' ? $result['status'] : $this->language->get('text_order_not_pay_status'),
	...
	public function info() {
		...
		$order_info = $this->model_account_order->getOrderWithStatus0($order_id);
		...
		if ($order_info) {
			...
			if($order_info['order_status_id'] == 0){
				...
				$data['payment'] = $this->load->controller('payment/' . $order_info['payment_code']);
				$this->language->load('account/orderext');
				$data['payment'] = str_replace($this->language->get('button_confirm'),$this->language->get('button_pay_continue'),$data['payment']);
			}else{
				$data['payment'] = '';
			}
