...
*/
	...
	public function productlist() {
		...
		if ($this->cart->hasShipping()) {
			// Validate if shipping address has been set.
			if (!isset($this->session->data['shipping_address'])) {
				$error = $this->language->get('error_not_select_shipping_address');
			}

			// Validate if shipping method has been set.
			if (!isset($this->session->data['shipping_method'])) {
				$error = $this->language->get('error_not_select_shipping_method');
			}
		} else {
			unset($this->session->data['shipping_address']);
			unset($this->session->data['shipping_method']);
		...
		// Validate cart has products and has stock.
		if ((!$this->cart->hasProducts() && empty($this->session->data['vouchers']) && empty($this->session->data['recharges'])) || (!$this->cart->hasStock() && !$this->config->get('config_stock_checkout'))) {
			$error = $this->language->get('error_product_no_stock_or_cart_empty');
		}
		...
		foreach ($products as $product) {
			...
			if ($product['minimum'] > $product_total) {
				$error = $this->language->get('error_not_minimum_quantity');
		...
		if (file_exists(DIR_TEMPLATE . $this->config->get('config_template') . '/template/checkout/product_list.tpl')) {
			$this->response->setOutput($this->load->view($this->config->get('config_template') . '/template/checkout/product_list.tpl', $data));
		} else {
			$this->response->setOutput($this->load->view('default/template/checkout/product_list.tpl', $data));
		}
	}
}
