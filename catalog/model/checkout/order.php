...
class ModelCheckoutOrder extends Model {
	...
	public function editOrder($order_id, $data) {
		...
		// Recharge
		$this->load->model('total/recharge');
	...
	public function addOrderHistory($order_id, $order_status_id, $comment = '', $notify = false, $override = false) {
		...
		if ($order_info) {
			...
			if (!in_array($order_info['order_status_id'], $this->config->get('config_complete_status')) && in_array($order_status_id, $this->config->get('config_complete_status'))) {
				...
				$recharge_infos = $this->model_account_order->getOrderRecharges($order_id);

				$this->load->model('account/transaction');
				foreach($recharge_infos as $recharge_info){
					$this->model_account_transaction->addTransaction($order_info['customer_id'], 'ORDER ID:'.$order_id, $recharge_info['amount'], $order_id, $recharge_info['order_recharge_id']);
				}
			}
			// If current order status is complete but new status is not complete then remove the customer transaction balance
			if (in_array($order_info['order_status_id'], $this->config->get('config_complete_status')) && !in_array($order_status_id, $this->config->get('config_complete_status'))) {
				$this->load->model('account/transaction');
				$this->model_account_transaction->deleteTransaction($order_id);
			}
			...
			if (!$order_info['order_status_id'] && $order_status_id) {
				...
				$order_recharge_query = $this->db->query("SELECT * FROM " . DB_PREFIX . "order_recharge WHERE order_id = '" . (int)$order_id . "'");

				foreach ($order_recharge_query->rows as $recharge) {
					$data['recharges'][] = array(
				...
				foreach ($order_recharge_query->rows as $recharge) {
					$text .= '1x ' . $recharge['description'] . ' ' . $this->currency->format($recharge['amount'], $order_info['currency_code'], $order_info['currency_value']);
				}
