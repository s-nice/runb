...
class ModelAccountOrder extends Model {
	...
	public function getOrderWithStatus0($order_id) {
	...
	public function getOrdersWithStatus0($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$hours = $this->config->get('config_unpaid_time');
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, o.shipping_firstname, o.shipping_lastname, os.name as status, o.order_status_id, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND ((o.order_status_id > '0' AND os.language_id = '" . (int)$this->config->get('config_language_id') . "') OR (o.order_status_id = '0' AND o.date_added > date_sub(now(),INTERVAL $hours HOUR ))) ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}

	public function getTotalOrdersWithStatus0() {
		$hours = $this->config->get('config_unpaid_time');
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$this->customer->getId() . "' AND (order_status_id > '0' OR (order_status_id = '0' AND date_added > date_sub(now(),INTERVAL $hours HOUR )))");

		return $query->row['total'];
	}

	public function getOrdersOfStatus0($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$hours = $this->config->get('config_unpaid_time');
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, o.shipping_firstname, o.shipping_lastname, os.name as status, o.order_status_id, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND o.order_status_id = '0' AND o.date_added > date_sub(now(),INTERVAL $hours HOUR ) ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}

	public function getTotalOrdersOfStatus0() {
		$hours = $this->config->get('config_unpaid_time');
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$this->customer->getId() . "' AND order_status_id = '0' AND date_added > date_sub(now(),INTERVAL $hours HOUR )");

		return $query->row['total'];
	}

	public function getOrdersOfStatusUnpaid($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$unpaid_status = $this->config->get('config_unpaid_status');
		$hours = $this->config->get('config_unpaid_time');
		$query = $this->db->query("SELECT o.order_id, o.firstname, o.lastname, o.shipping_firstname, o.shipping_lastname, os.name as status, o.order_status_id, o.date_added, o.total, o.currency_code, o.currency_value FROM `" . DB_PREFIX . "order` o LEFT JOIN " . DB_PREFIX . "order_status os ON (o.order_status_id = os.order_status_id) WHERE o.customer_id = '" . (int)$this->customer->getId() . "' AND (o.order_status_id = '0' OR o.order_status_id = '$unpaid_status') AND o.date_added > date_sub(now(),INTERVAL $hours HOUR ) ORDER BY o.order_id DESC LIMIT " . (int)$start . "," . (int)$limit);

		return $query->rows;
	}


	public function getTotalOrdersOfStatusUnpaid() {
		$unpaid_status = $this->config->get('config_unpaid_status');
		$hours = $this->config->get('config_unpaid_time');
		$query = $this->db->query("SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "order` WHERE customer_id = '" . (int)$this->customer->getId() . "' AND (order_status_id = '0' OR order_status_id = '$unpaid_status') AND date_added > date_sub(now(),INTERVAL $hours HOUR )");

		return $query->row['total'];
	}

	public function getOrdersOfStatusShipped($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
	...
	public function getTotalOrdersOfStatusShipped() {
	...
	public function getOrdersOfStatusUnshipped($start = 0, $limit = 20) {
		if ($start < 0) {
			$start = 0;
		}
		...
		$unshipped_status = $this->config->get('config_paid_status');//array_diff($this->config->get('config_processing_status'), array($this->config->get('config_shipped_status_id')));
	...
	public function getTotalOrdersOfStatusUnshipped() {
		$unshipped_status = $this->config->get('config_paid_status');//array_diff($this->config->get('config_processing_status'), array($this->config->get('config_shipped_status_id')));
